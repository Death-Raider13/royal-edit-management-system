...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const visibleNavigation = user.role === "admin" ? navigation : navigation.filter((item) => ["/", "/tasks", "/notifications"].includes(item.path));
  const active = visibleNavigation.find((item) => item.path === location)?.label ?? "Operations";
  return (
